...
<body>
    ...
    <section class="container-md mt-4 mb-5">
        ...
        <!--본문-->
        <c:if test="${type == 1}">
	        <form method="post" action="/editspeakm.do?seq=${dto.sm_seq}">
		        <div class="border border-1 border-secondary border-opacity-50 rounded p-4">
					
					<input type="hidden" id="sm_seq" name="sm_seq" value="${dto.sm_seq}">
					
		            <div class="fw-bold">제목</div>
		            <input class="form-control mt-2 w-50 mb-5" id="sm_subject" name="sm_subject" type="text" placeholder="제목" aria-label=".form-control-lg example" value="${dto.sm_subject}">
		
		            <div class="fw-bold">본문</div>
		            <div class="main-content-sel3">
		                 <textarea id="sm_content" name="sm_content">${dto.sm_content}</textarea>
		            </div>
		...
		<c:if test="${type == 2}">
	        <form method="post" action="/editspeakb.do?seq=${dto.sb_seq}">
		        <div class="border border-1 border-secondary border-opacity-50 rounded p-4">
					
					<input type="hidden" id="sb_seq" name="sb_seq" value="${dto.sb_seq}">
					
		            <div class="fw-bold">제목</div>
		            <input class="form-control mt-2 w-50 mb-5" id="sb_subject" name="sb_subject" type="text" placeholder="제목" aria-label=".form-control-lg example" value="${dto.sb_subject}">
		
		            <div class="fw-bold">본문</div>
		            <div class="main-content-sel3">
		                 <textarea id="sb_content" name="sb_content">${dto.sb_content}</textarea>
		            </div>
